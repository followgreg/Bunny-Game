const CACHE = 'bunnygame-v146';
const ASSETS = [
  '/',
  '/index.html',
  '/classic.html',
  '/hare-trigger.html',
  '/hare-brain.html',
  '/bomb-mode.html',
  '/86-bunnies.html',
  '/hare-line.html',
  '/shroom-mode.html',
  '/bunny-hop.html',
  '/cabbage-drop.html',
  '/cubrick.html',
  '/cubrick-puzzles.json',
  '/hexflip.html',
  '/cropped.html',
  '/flagged.html',
  '/wave.html',
  '/mascot-wave.html',
  '/pickler.html',
  '/manifest.json',
  '/assets/css/shared.css',
  '/assets/css/classic.css',
  '/assets/css/bomb-mode.css',
  '/assets/css/86-bunnies.css',
  '/assets/css/hare-line.css',
  '/assets/css/shroom-mode.css',
  '/assets/css/bunny-hop.css',
  '/assets/css/cabbage-drop.css',
  '/assets/css/hare-trigger.css',
  '/assets/css/hare-brain.css',
  '/assets/css/cropped.css',
  '/assets/css/flagged.css',
  '/assets/css/wave.css',
  '/assets/css/mascot-wave.css',
  '/assets/css/pickler.css',
  '/assets/css/cubrick.css',
  '/assets/logos/classic_logo.svg',
  '/assets/logos/cubrick_logo.svg',
  '/assets/logos/cropped_logo.svg',
  '/assets/logos/flagged_logo.svg',
  '/assets/logos/wave_logo.svg',
  '/assets/logos/mascot_logo.svg',
  '/assets/logos/pickler_logo.svg',
  '/assets/css/hexflip.css',
  '/assets/js/nav.js',
  '/assets/js/shared.js',
  '/assets/js/classic.js',
  '/assets/js/bomb-mode.js',
  '/assets/js/86-bunnies.js',
  '/assets/js/hare-line.js',
  '/assets/js/shroom-mode.js',
  '/assets/js/bunny-hop.js',
  '/assets/js/cabbage-drop.js',
  '/assets/js/hare-trigger.js',
  '/assets/js/hare-brain.js',
  '/assets/js/cropped.js',
  '/assets/js/flagged.js',
  '/assets/js/wave.js',
  '/assets/js/mascot-wave.js',
  '/assets/js/pickler.js',
  '/pickler/nose.svg',
  '/pickler/pickle.svg',
  '/whiskers.html',
  '/assets/css/whiskers.css',
  '/assets/js/whiskers.js',
  '/assets/data/schools.json',
  '/assets/data/capitals.json',
  '/assets/js/cubrick.js',
  '/assets/js/hexflip.js',
  '/assets/icons/hexflip-logo.svg',
  '/assets/logos/Hexflip_Logo.svg',
  '/assets/logos/noon_logo.svg',
  '/assets/logos/scramble_logo.svg',
  '/snek.html',
  '/assets/css/snek.css',
  '/assets/js/snek.js',
  '/assets/data/snek-levels.json',
  '/mobi.html',
  '/assets/css/mobi.css',
  '/assets/js/mobi.js',
  '/assets/data/mobi-levels.json',
  '/assets/logos/mobi_logo.svg',
  '/mobi-source/shape-01.svg',
  '/mobi-source/shape-02.svg',
  '/mobi-source/shape-03.svg',
  '/mobi-source/shape-04.svg',
  '/mobi-source/shape-05.svg',
  '/mobi-source/shape-06.svg',
  '/mobi-source/shape-07.svg',
  '/mobi-source/shape-08.svg',
  '/mobi-source/shape-09.svg',
  '/mobi-source/shape-10.svg',
  '/next.html',
  '/assets/css/next.css',
  '/assets/js/next.js',
  '/assets/data/next-boards.json',
  '/assets/logos/next_logo.svg',
  '/honey.html',
  '/assets/css/honey.css',
  '/assets/js/honey.js',
  '/assets/data/honey-levels.json',
  '/assets/logos/honey_logo.svg',
  '/solo.html',
  '/assets/css/solo.css',
  '/assets/js/solo.js',
  '/assets/data/solo-levels.json',
  '/assets/logos/solo_logo.svg',
  '/lava.html',
  '/assets/css/lava.css',
  '/assets/js/lava.js',
  '/assets/data/lava-levels.json',
  '/sixty.html',
  '/assets/css/sixty.css',
  '/assets/js/sixty.js',
  '/assets/logos/sixty_logo.svg',
  '/warren.html',
  '/assets/css/warren.css',
  '/assets/js/warren.js',
  '/assets/data/warren-boards.json',
  '/assets/logos/warren_logo.svg',
  '/tonal.html',
  '/assets/css/tonal.css',
  '/assets/js/tonal.js',
  '/assets/logos/tonal_logo.svg',
  '/assets/og-images/tonal-og.png',
  '/recall.html',
  '/assets/css/recall.css',
  '/assets/js/recall.js',
  '/assets/logos/RECALL_LOGO.svg',
  '/assets/og-images/recall-og.png',
  '/spectrum.html',
  '/assets/css/spectrum.css',
  '/assets/js/spectrum.js',
  '/assets/logos/spectrum_logo.svg',
  '/assets/og-images/spectrum-og.png',
  '/shaded.html',
  '/assets/css/shaded.css',
  '/assets/js/shaded.js',
  '/assets/data/shaded-levels.json',
  '/assets/logos/shaded_logo.svg',
  '/assets/og-images/shaded-og.png',
  '/assets/og-images/lava-og.png',
  '/assets/logos/lava_logo.svg',
  '/lexi.html',
  '/assets/css/lexi.css',
  '/assets/js/lexi.js',
  '/assets/data/lexi-questions.json',
  '/assets/logos/lexi_logo.svg',
  '/assets/og-images/lexi-og.png',
  '/assets/logos/haretrigger_logo.svg',
  '/assets/logos/harebrain_logo.svg',
  '/assets/logos/bombmode_logo.svg',
  '/assets/logos/86bunnies_logo.svg',
  '/assets/logos/hareline_logo.svg',
  '/assets/logos/shroommode_logo.svg',
  '/assets/logos/bunnyhop_logo.svg',
  '/assets/logos/cabbagedrop_logo.svg',
  '/assets/logos/whiskers_logo.svg',
  '/assets/og-images/hare-trigger-og.png',
  '/assets/og-images/hare-brain-og.png',
  '/assets/og-images/bomb-mode-og.png',
  '/assets/og-images/86-bunnies-og.png',
  '/assets/og-images/hare-line-og.png',
  '/assets/og-images/shroom-mode-og.png',
  '/assets/og-images/bunny-hop-og.png',
  '/assets/og-images/cabbage-drop-og.png',
  '/assets/og-images/whiskers-og.png',
  '/proof.html',
  '/assets/css/proof.css',
  '/assets/js/proof.js',
  '/assets/data/proof-index.json',
  '/assets/logos/proof_logo.svg',
  '/assets/og-images/proof-og.png',
  '/colorblind.html',
  '/assets/css/colorblind.css',
  '/assets/js/colorblind.js',
  '/assets/data/colorblind-levels.json',
  '/assets/logos/colorblind_logo.svg',
  '/niner.html',
  '/assets/css/niner.css',
  '/assets/js/niner.js',
  '/colorbomb-start.html',
  '/colorbomb.html',
  '/assets/css/colorbomb.css',
  '/assets/js/colorbomb.js',
  '/assets/logos/niner_logo.svg',
  '/assets/logos/colorbomb_logo.svg',
  '/assets/og-images/niner-og.png',
  '/wordup.html',
  '/assets/css/wordup.css',
  '/assets/js/wordup.js',
  '/assets/logos/wordup_logo.svg',
  '/assets/data/words.txt',
  '/assets/og-images/wordup-og.png',
  '/threaded.html',
  '/assets/css/threaded.css',
  '/assets/js/threaded.js',
  '/assets/logos/threaded_logo.svg',
  '/assets/og-images/threaded-og.png',
  '/fuse.html',
  '/assets/css/fuse.css',
  '/assets/js/fuse.js',
  '/assets/data/fuse-levels.json',
  '/assets/logos/fuse_logo.svg',
  '/race-to-zero.html',
  '/assets/css/race-to-zero.css',
  '/assets/js/race-to-zero.js',
  '/assets/logos/racetozero_logo.svg',
  '/assets/logos/POISE_LOGO.svg',
  '/assets/icons/blue-bunny.svg',
  '/assets/icons/red-bunny.svg',
  '/assets/icons/mushroom.svg',
  '/assets/icons/cabbage.svg',
  '/assets/icons/carrot.svg',
  '/Icons/BunnyGameLogo.svg',
  '/Icons/icon-192.png',
  '/Icons/icon-512.png',
];

self.addEventListener('install', e => {
  e.waitUntil(
    caches.open(CACHE).then(c => c.addAll(ASSETS))
  );
  self.skipWaiting();
});

self.addEventListener('activate', e => {
  // Remove old caches on update
  e.waitUntil(
    caches.keys().then(keys =>
      Promise.all(keys.filter(k => k !== CACHE).map(k => caches.delete(k)))
    )
  );
  self.clients.claim();
});

self.addEventListener('fetch', e => {
  // Network-first for HTML navigation — always serve fresh pages
  if (e.request.mode === 'navigate') {
    e.respondWith(
      fetch(e.request).catch(() => caches.match(e.request))
    );
    return;
  }
  // Cache-first for all other assets (JS, CSS, images)
  e.respondWith(
    caches.match(e.request).then(r => r || fetch(e.request))
  );
});
